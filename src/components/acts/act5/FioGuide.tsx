    ...
    clat percentiles (usec):
     ...
     | 99.95th=[  668], 99.99th=[ 1942]`}
        ...
        <RevealCard
          ...
          options={["Higher queue depth reduces per-command PCIe protocol overhead through better packet batching", "At QD128 the 128 in-flight requests let the controller dispatch reads across all NAND channels and dies in parallel — multiplying throughput while each individual read takes the same time", "QD128 activates the SSD turbo mode which overclocks the NAND interface", "Higher QD improves IOPS because the controller reorders commands to read sequential pages converting random to sequential"]}
          correctIndex={1}
        />
      </div>
    </SectionWrapper>
