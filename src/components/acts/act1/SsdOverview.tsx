      ...
      <svg viewBox="0 0 620 280" className="w-full max-w-3xl mx-auto" fill="none">
        ...
        {[0, 1, 2, 3].map((ch) => (
          ...
          >
            {/* Channel group */}
            <rect
              x={400 + ch * 56} y="105" width="50" height="150" rx="6"
              fill="#f5a62306" stroke="#f5a62340" strokeWidth="1"
            />
            <text
              x={425 + ch * 56} y="120" textAnchor="middle"
              className="fill-nvme-amber text-[7px] font-mono font-bold"
            >
              CH{ch}
            </text>

            {/* Two NAND dies per channel */}
            {[0, 1].map((die) => (
              <rect
                key={die}
                x={406 + ch * 56} y={128 + die * 58} width="38" height="50" rx="4"
                fill="#f5a62308" stroke="#f5a623" strokeWidth="1"
              />
            ))}
            {[0, 1].map((die) => (
              <g key={`label-${die}`}>
                <text
                  x={425 + ch * 56} y={148 + die * 58} textAnchor="middle"
                  className="fill-nvme-amber text-[7px] font-bold"
                >
                  NAND
                </text>
                <text
                  x={425 + ch * 56} y={160 + die * 58} textAnchor="middle"
                  className="fill-text-muted text-[6px]"
                >
                  Die {ch * 2 + die}
                </text>
                <text
                  x={425 + ch * 56} y={170 + die * 58} textAnchor="middle"
                  className="fill-text-muted text-[5px]"
                >
                  TLC/QLC
                </text>
              </g>
        ...
        {/* Arrow controller → NAND channels */}
        <motion.line
    ...
    <SectionWrapper className="py-24 px-4 bg-story-surface">
      <div className="max-w-4xl mx-auto">
        ...
        <p className="text-text-secondary mb-8 leading-relaxed max-w-3xl">
          ...
          controller through the PCIe bus (covered in the next act), and the controller
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
          ...
          <div className="bg-story-card rounded-2xl p-5 card-shadow">
            ...
            <p className="text-text-muted text-xs">
              ...
              the maximum bandwidth between the host and the SSD controller. These are covered in
              detail in the PCIe section next.
            </p>
          </div>
        </div>
      </div>
